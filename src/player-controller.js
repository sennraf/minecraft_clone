...
export const player_controller = (() => {
  ...
  class PlayerController extends entity.Component {
    ...
    _FindIntersections(boxes, position) {
      // Cylinder dimensions
      const radius = 0.5; // Width/2 for 1 unit width
      const height = 2.0; // Total height
      const halfHeight = height / 2;

      const intersections = boxes.filter(b => {
        // Get closest point on box to cylinder center line
        const boxClosest = new THREE.Vector3();
        b.clampPoint(position, boxClosest);
        
        // First check height
        if (Math.abs(boxClosest.y - position.y) > halfHeight) {
          return false;
        }

        // Then check radius (horizontal distance)
        const horizontalDist = new THREE.Vector2(
          boxClosest.x - position.x,
          boxClosest.z - position.z
        ).length();
